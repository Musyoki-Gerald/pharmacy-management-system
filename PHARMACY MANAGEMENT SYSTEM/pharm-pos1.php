...
<body>
    ...
    <?php
			...
			if (isset($_POST['cid'])) {
				...
				mysqli_stmt_bind_param($stmt, 'ss', $cid, $_SESSION['user']);
...
<input type="text" name="mdname" value="<?php echo isset($row4[1]) ? htmlspecialchars($row4[1]) : ''; ?>" readonly><br><br>
...
<input type="text" name="mcat" value="<?php echo isset($row4[3]) ? htmlspecialchars($row4[3]) : ''; ?>" readonly><br><br>

					
<label for="mloc">Location:</label>
<input type="text" name="mloc" value="<?php echo isset($row4[5]) ? htmlspecialchars($row4[5]) : ''; ?>" readonly><br><br>
...
if (isset($_POST['add'])) {
    ...
    if ($qty > $aqty || $qty == 0) {
        ...
        $qry6 = "INSERT INTO sales_items (sale_id, med_id, sale_qty, tot_price) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $qry6);
        mysqli_stmt_bind_param($stmt, 'iiid', $sid, $mid, $qty, $price);
